...
[McpServerToolType]
public sealed class ExplainSymbolTool(ICodeUnderstandingService codeUnderstandingService)
{
    ...
    [Description(
        """
        Use this tool when you need to understand what a specific symbol (type, method, property, field, etc.) 
        does, what its signature looks like, where it is used in the codebase, and what XML documentation it 
        already exposes. It provides a human-readable explanation along with impact hints showing areas with 
    ...
    public Task<ExplainSymbolResult> ExecuteAsync(CancellationToken cancellationToken,
        ...
        int? column = null)
    {
        return _codeUnderstandingService.ExplainSymbolAsync(symbolId.ToExplainSymbolRequest(path, line, column),
            cancellationToken);
    }
}
